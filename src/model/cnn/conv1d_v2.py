...
from src.utils.tensor_util import gelu, get_activation, create_initializer, loop_slice
...
def Conv1D(input_tensor, filter_shape, stride, name, padding='SAME'):
    """
    ...
    slice_channels = tf.concat(  # [B, S, H, O, W]
        [tf.expand_dims(
            _tensor, axis=-2) for _tensor in loop_slice(
            input_tensor, stride=channel_stride,
            width=channel_win_size,
            num=out_channels, axis=-1)], axis=-2)
    slice_contents = tf.concat(  # [B, S, F, H, O, W]
        [tf.expand_dims(
            _tensor, axis=1) for _tensor in loop_slice(
            slice_channels, stride=stride,
